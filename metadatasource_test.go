...
// TestMetadataInjectionSeam proves that metadata.Use swaps the data the public,
...
func TestMetadataInjectionSeam(t *testing.T) {
	...
	mc, err := metadata.NewContainer(coll, ccToRegion)
	require.NoError(t, err)
	restore := metadata.Use(mc)
	...
	// metadata.NewContainer rejects an empty collection.
	_, err = metadata.NewContainer(&PhoneMetadataCollection{}, nil)
