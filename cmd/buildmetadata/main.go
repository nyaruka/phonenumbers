...
func buildMetadata() error {
	...
	fmt.Printf("Wrote metadataversion.go (MetadataVersion = %q)\n", version)
...
`
	...
	if err := os.WriteFile("metadataversion.go", []byte(fmt.Sprintf(tmpl, version)), 0664); err != nil {
		return fmt.Errorf("error writing metadataversion.go: %w", err)
	}
